...
def display(data):
    ...
    if(cmd == "ERR"):
        ...
        data = words[2] + display_rank(words[1]) + " > " + (' '.join(data for data in words[3:]))
    elif(cmd == "PRV_MSG"):
        data = words[2] + display_rank(words[1]) + ' (vous chuchute) > ' + (' '.join(data for data in words[3:]))
    elif(cmd == "LIST"):
        ...
        if(words[2]=="0"):
            if(words[3] == nick):
                ...
                data = words[3] + " a rejoint le channel."
        else:
            data = "Vous venez de créer le channel " + words[1]
    elif(cmd == "KICK"):
        if(words[3]==nick):
            data = words[1] + " (*) vous a kické !"
        else:
            if(words[1]==nick):
